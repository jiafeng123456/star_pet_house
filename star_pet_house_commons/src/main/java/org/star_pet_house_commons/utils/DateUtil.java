...
public class DateUtil extends DateUtils {
    ...
    /**
     * 自动判断日期字符串的格式，返回Date对象  {@link parseDateStrictly}
    ...
    public static Date parseDateStrictly(String dateString, String ... dateFormat) throws ParseException {
        ...
        if (dateFormat == null || dateFormat.length == 0) {
            return DateUtils.parseDateStrictly(dateString, DEFAULT_FORMATS);
        } else {
            return DateUtils.parseDateStrictly(dateString, dateFormat);
        }
    }
    ...
    /**
     * 将字符串转换成Date类型{@link parseDateStrictly }
     * @param dateString
     * @param dateFormat
     * @return
     */
    ...
    /**
     * 判断指定日期是星期几{@link getWeekStrictly}
     * @param time 要判断的日期
    ...
    public static int getWeekStrictly(String time, String format) throws ParseException {
        return getWeek(DateUtils.parseDateStrictly(time, format));
    }
    ...
    public static java.sql.Timestamp parse2TimestampStrictly(String dateString, String format) throws ParseException {
        return new java.sql.Timestamp(DateUtils.parseDateStrictly(dateString, format).getTime());
    }
    /**
     * 获取两个时间的间隔,字符串表示
     * @author huadi
     ...
     * @return
     */
    public static String getMaxCurrentDate(Date date){
        ...
        return DateUtil.format(calendarEnd.getTime(),DateUtil.DATE_TIME_FORMAT);
    }

    /**
     * 将时间格式TIMESTAMP_FORMAT转换成指定的时间格式
     * @param dateString
     * @return
     * @throws ParseException
     */
    public static String parseStr(String dateString,String dateFormat) throws ParseException {
        if (StringUtils.isEmpty(dateString)) {
            return null;
        }
        return DateUtil.format(DateUtils.parseDate(dateString,TIMESTAMP_FORMAT), dateFormat);
    }
